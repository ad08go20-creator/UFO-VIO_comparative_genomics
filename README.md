...
### Comparative completeness heatmap (multi-genome)

For comparing KEGG module completeness across several genomes at once
(rather than one genome's pathway bar plot), use Anvi'o's own metabolism
estimation instead of KEGGaNOG:

```bash
anvi-estimate-metabolism -e external-genomes.txt --matrix-format -O METABOLISM
```

This produces `METABOLISM-module_stepwise_completeness-MATRIX.txt`
(modules as rows, genomes as columns, values 0–1). Module IDs can be
mapped to readable names via the `module_names.txt` file Anvi'o ships
alongside its KEGG data directory.

[`scripts/python/kegg_comparative_heatmap.py`](scripts/python/kegg_comparative_heatmap.py)
renders this matrix as a heatmap, with the same gridline/border styling
used across every comparative figure in this project (see
[`chapter2_figures/`](chapter2_figures/) for an example output comparing
eight *Chromobacterium amazonense* genomes).
...
### ⚠️ Normalization is batch-relative — comparing genomes across separate runs

PGPg_finder's own `heatmap.py` normalizes each genome's gene counts against
the **combined total of every genome submitted in that same batch run**
(`total_sum = df_pivot.sum().sum()`). This means the "normalized" output
from a single-genome run and from a larger multi-genome batch run are on
incompatible scales and **cannot be merged directly** — a genome run alone
will always show ~100% of itself, while the same genome inside a
158-genome batch will show a tiny fraction of the batch total.

If you need to compare a genome that was annotated separately from the
rest of your comparison set, re-run PGPg_finder on all genomes together,
or recompute percentages from the **raw, non-normalized** gene counts
using only your comparison subset as the denominator.

### Comparative heatmap (multi-genome)

[`scripts/python/pgpt_comparative_heatmap.py`](scripts/python/pgpt_comparative_heatmap.py)
renders a merged raw-count table (see the normalization caveat above) as a
heatmap, styled consistently with the other comparative figures in this
project — see [`chapter2_figures/`](chapter2_figures/) for an example
output comparing eight *Chromobacterium amazonense* genomes at the Lv3
category level.